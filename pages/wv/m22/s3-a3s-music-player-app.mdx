...
- **1st:** There are more than just one piece of information (for example: `isPlaying`, `currentTrackIndex`) we need to share across the app, therefore, let's put all the information in one object – `defaulValues`.
...
:fire: Congratulations on making it this far down the rabbit hole – you are almost there, keep on!
