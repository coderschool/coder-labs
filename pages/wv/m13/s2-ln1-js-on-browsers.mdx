...
All of the elements, attributes, and text on a web page can be accessed by scripts using the DOM (Document Object Model).
...
**Script placement**: For most scripts, the end of the document, just before the `&lt;/body&gt;` tag, is the preferred placement because the browser will be done parsing the document and its DOM structure.
...
![node tree](../../assets/wv/m13/dom_node_tree.png)

The nodes within the first `&lt;p&gt;` element:

![node tree 2](../../assets/wv/m13/dom_node_tree_2.png)
...
Then `paragraphs` is a collection of nodes. `paragraphs[0]` is a reference to the first paragraph in the document, `paragraphs[1]` refers to the second, and so on.
...
```javascript
const firstColumn = document.getElementsByClassName("column-a");
```
...
```javascript
const sidebarPara = document.querySelectorAll(".sidebar p");
...
```javascript
const catImage = document.getElementById("cat-photo");
alert(catImage.getAttribute("src")); // Alerts "photo.jpg".
```

### Manipulating Nodes

Once we’ve accessed a node using one of the previously discussed methods, the DOM gives us several built-in methods for manipulating those elements, their attributes, and their contents.
...
```javascript
const introDiv = document.getElementsByClassName("intro");
...
```javascript
const ourText = document.createTextNode("This is our text.");
```
...
```javascript
const ourDiv = document.getElementById("our-div");
const newParagraph = document.createElement("p");
const copy = document.createTextNode("Hello, world!");
...
```javascript
const ourDiv = document.getElementById("our-div");
const para = document.getElementById("our-paragraph");
const newHeading = document.createElement("h1");
const headingText = document.createTextNode("A new heading");
...
```javascript
const ourDiv = document.getElementById("our-div");
const swapMe = document.getElementById("swap-me");
...
An event is an action that can be detected with JavaScript, such as when the user clicks an element or just moves the mouse over it. There are 3 ways to register event handlers for a DOM element.
...
- Using <a href="https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener" target="_blank">addEventListener()</a>
