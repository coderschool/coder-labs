...
# Quiz Solutions - State Management with Redux
...
- [x] Manage states.
- [ ] Manage UI rendering
- [ ] Manage the application's logic.
...
- [x] States are read-only: You can't modify the state directly – the only way to change the state is to dispatch an action.
- [x] Changes are made with pure functions: Reducer or a pure function that takes the previous state and action and then returns the new state.
...
### 8. In Redux Toolkit, `createAsyncThunk()` is used to fetch data from API and store the response in Redux states. How is an AsyncThunk added to the slice object?
...
### 10. You will need to export `slice.reducer` in `slice.js` and import it to `store.js` to add the reducer into your `store`.
