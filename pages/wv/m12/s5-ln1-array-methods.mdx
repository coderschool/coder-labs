...
Read more about why we use one over the other [here](https://alligator.io/js/foreach-vs-for-loops/).
...
`shift()` is a method just like `pop()` but instead of removing the last element from an array, `shift()` removes the first element. Read more about shift [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/shift)
